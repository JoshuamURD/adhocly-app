...
final class WorkspaceTests: XCTestCase {
    ...
    @MainActor func testNestedFoldersCollapsePersistAndProjectDeletionKeepsTasks() {
        continueAfterFailure = false
        let app = XCUIApplication()
        app.launch()
        app.tabBars.buttons["Projects"].tap()
        let suffix = UUID().uuidString.prefix(6)
        let parent = "Work \(suffix)", child = "Clients \(suffix)", project = "Website \(suffix)"
        app.buttons["New folder"].tap()
        let name = app.textFields["Folder name"]
        XCTAssertTrue(name.waitForExistence(timeout: 4))
        name.tap()
        name.typeText(parent)
        app.buttons["Create folder"].tap()
        let parentLabel = app.staticTexts[parent].firstMatch
        XCTAssertTrue(parentLabel.waitForExistence(timeout: 4))
        parentLabel.press(forDuration: 1)
        app.buttons["New subfolder"].tap()
        XCTAssertTrue(name.waitForExistence(timeout: 4))
        name.tap()
        name.typeText(child)
        app.buttons["Create folder"].tap()
        let childLabel = app.staticTexts[child].firstMatch
        XCTAssertTrue(childLabel.waitForExistence(timeout: 4))
        app.buttons["New project"].tap()
        app.textFields["Project name"].tap()
        app.textFields["Project name"].typeText(project)
        app.buttons["Create project"].tap()
        app.staticTexts[project].firstMatch.tap()
        let input = app.textFields["Quick capture. Exclamation mark for due date, at sign for planned date, slash for project."]
        XCTAssertTrue(input.waitForExistence(timeout: 4))
        input.tap()
        input.typeText("Keep \(suffix)")
        app.buttons["Add"].tap()
        app.buttons["More actions"].tap()
        app.buttons["Move to folder"].tap()
        app.buttons["\(parent) / \(child)"].tap()
        app.navigationBars.buttons["Projects"].tap()
        XCTAssertTrue(childLabel.waitForExistence(timeout: 4))
        XCTAssertTrue(app.staticTexts[project].firstMatch.exists)
        childLabel.tap()
        XCTAssertFalse(app.staticTexts[project].firstMatch.exists)
        parentLabel.tap()
        XCTAssertFalse(childLabel.exists)
        screenshot("iPhone — Collapsed project folders", app: app)
        app.terminate()
        app.launch()
        app.tabBars.buttons["Projects"].tap()
        XCTAssertTrue(parentLabel.waitForExistence(timeout: 4))
        ...
        childLabel.tap()
        app.staticTexts[project].firstMatch.tap()
        XCTAssertTrue(app.buttons["Complete Keep \(suffix)"].waitForExistence(timeout: 4))
        app.buttons["More actions"].tap()
        app.buttons["Delete project"].tap()
        app.buttons["Cancel"].tap()
        XCTAssertTrue(app.buttons["Complete Keep \(suffix)"].exists)
        app.buttons["More actions"].tap()
        app.buttons["Delete project"].tap()
        app.buttons["Delete “\(project)”"].tap()
        XCTAssertTrue(app.buttons["New project"].waitForExistence(timeout: 4))
        XCTAssertFalse(app.staticTexts[project].firstMatch.exists)
        app.staticTexts["Inbox"].firstMatch.tap()
        XCTAssertTrue(app.buttons["Complete Keep \(suffix)"].waitForExistence(timeout: 4))
        app.buttons["More actions"].tap()
        XCTAssertFalse(app.buttons["Delete project"].exists)
    }
